...
use idlib::{
    AuthCallback, AuthorizationRejection, Authorizations, Authorize, IdpClient, PermissionResponse,
    SecretKey, Variables,
};
...
pub async fn api_route(
    ...
    if let Err(e) = fill_auth(&authorizations, &client, &variables, &secret_key).await {
        log::error!("Failed to fetch authorization info");
    }
